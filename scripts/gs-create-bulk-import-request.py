...
def bulk_import(bucket_path):
    ...
    result = subprocess.run(['gsutil', 'ls', '-r', bucket_path], capture_output=True, text=True)
    if result.returncode != 0:
        raise RuntimeError(f"gsutil command failed: {result.stderr}")

    # process the output
    ndjson_files = [line for line in result.stdout.splitlines() if line.endswith('.ndjson')]
